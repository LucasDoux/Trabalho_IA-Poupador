...
public class Poupador extends ProgramaPoupador {
	...
	//Coordenadas em X de acordo com a representação visual do agente-poupador(tabela visual do poupador)
	...
	//Coordenadas em Y de acordo com a representação visual do agente-poupador(tabela visual do poupador)
	...
	PossibilidadeMovimento getLugarMenosExplorado(){
		...
		if(acima.isViavel && memoria[acima.x][acima.y] == menorPeso){
			if(acima.visao == Visao.PASTINHA_PODER){
				System.out.println(" PASTINHA ADQUIRIDA !");
			}
			if(aceitaPastilha){
		...
		if(esquerda.isViavel && memoria[esquerda.x][esquerda.y] == menorPeso){
			if(aceitaPastilha){
				...
				if(esquerda.visao != Visao.PASTINHA_PODER){
					opcoesIguais.add(esquerda);
				}
			}
		}
		if(direita.isViavel && memoria[direita.x][direita.y] == menorPeso){
			if(aceitaPastilha){
				...
				if(direita.visao != Visao.PASTINHA_PODER){
					opcoesIguais.add(direita);
				}
			}
		}
		if(baixo.isViavel && memoria[baixo.x][baixo.y] == menorPeso){
			if(aceitaPastilha){
				...
				if(baixo.visao != Visao.PASTINHA_PODER){
					opcoesIguais.add(baixo);
				}
			}
		}
	...
	boolean movimentoIsViavel(int coordenadaVisao){
		return getResultVisao(sensor.getVisaoIdentificacao()[coordenadaVisao]) == Visao.CELULA_VAZIA || getResultVisao(sensor.getVisaoIdentificacao()[coordenadaVisao]) == Visao.MOEDA  || getResultVisao(sensor.getVisaoIdentificacao()[coordenadaVisao]) == Visao.BANCO || getResultVisao(sensor.getVisaoIdentificacao()[coordenadaVisao]) == Visao.PASTINHA_PODER;
	}
